...
class YTDL{
  ...
  static download_twitch(clip) {
    return YTDL.info_twitch(clip).then(function(Info){
      return Request(Info)
    })
  }
  ...
  static info_twitch(clip){
    ...
    let DL = new Promise(function(Resolve, Reject){
      ...
      let Process = ChildProcess.spawn('youtube-dl', ['-g', '--cookies', CookiePath, '-o', CookiePath, url]);
      let Timeout
